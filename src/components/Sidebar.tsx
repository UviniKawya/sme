...
const navItems: { id: Page; label: string; icon: React.ReactNode }[] = [
  ...
  {
    id: "assessment",
    label: "Digital Readiness Assessment",
    icon: (
      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
...
export default function Sidebar() {
  const { currentPage, setCurrentPage, setAssessmentSubPage } = useAppContext();

  const handleNav = (page: Page) => {
    setCurrentPage(page);
    if (page === "assessment") {
      setAssessmentSubPage("overview");
    }
  };
            ...
            <li key={item.id}>
              <button
                ...
                className={`sidebar-item w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm ${
                  currentPage === item.id
                    ? "active text-brand-600 bg-brand-50 font-semibold"
